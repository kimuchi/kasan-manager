...
ROOT = Path(__file__).resolve().parents[1]  # worktree root
PRODUCT_ROOT = ROOT
...
def test_existing_pdf_summary_intact():
    """既存PDF検出サマリが壊れていないこと"""
    out_md = PRODUCT_ROOT / "out/_test_dsl_houmon_kango.md"
    rc, log, md = run_judge(
...
def test_dsl_section_appears():
    """要件ロジック評価セクションが出る"""
    out_md = PRODUCT_ROOT / "out/_test_dsl_section.md"
    rc, log, md = run_judge(
        "--domain", "kaigo", "--service", "houmon_kango_kaigo", "--office", "DEMO-0007",
        "--receipt-pdf", str(PRODUCT_ROOT / "tests/fixtures/houmon_kango_kaigo_receipt_sample.pdf"),
        "--evidence-out", str(PRODUCT_ROOT / "tenant_data/evidence/DEMO-0007/"),
...
def test_not_applicable_not_in_opportunity():
    ...
    out_md = PRODUCT_ROOT / "out/_test_dsl_not_applicable.md"
    rc, log, md = run_judge(
        "--domain", "kaigo", "--service", "houmon_kango_kaigo", "--office", "DEMO-0007",
        "--receipt-pdf", str(PRODUCT_ROOT / "tests/fixtures/houmon_kango_kaigo_receipt_sample.pdf"),
        "--evidence-out", str(PRODUCT_ROOT / "tenant_data/evidence/DEMO-0007/"),
...
def test_no_forbidden_terms_in_public_md():
    ...
    public_files = [
        PRODUCT_ROOT / "out/public_release_note.md",
        PRODUCT_ROOT / "README_PUBLIC.md",
    ] + list((PRODUCT_ROOT / "out").glob("sample_*_public.md"))
    hits = []
