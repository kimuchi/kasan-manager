...
> **訪問介護** | 事業所コード `DEMO-0005` | 生成日時 `2026-05-11T02:19:07`
...
- ソースファイル: `houmon_kaigo_receipt_sample.pdf`
- 抽出日時: 2026-05-06T21:59:25
- 抽出版: `v2026.05.06-alpha.4.4`
- 推定利用者数: **6名**
- 要介護度分布: 要介護1: 1名 / 要介護2: 1名 / 要介護3: 1名 / 要介護4: 1名 / 要介護5: 2名
- 要介護3以上割合: **66.7%**
- 抽出信頼度: `high`
...
| 📄 PDFで算定中として検出 | 5 | レセプトPDFから算定中と推定された加算 |
| 　└ うち要件確認済 | 0 | 要件マスタとも整合（自信度高） |
| 　└ うち要件未確認 | 5 | 算定中だが要件マスタは未確認 |
| 📄❔ PDFから未検出だが取得候補 | 1 | PDF未検出≠未算定。要追加確認 |
...
| 加算 | PDF検出件数 | 要件状態 |
|---|---:|---|
| 特定事業所加算(II) (`tokutei_jigyousho_II`) | 6件 | 💰❔ 算定中（要件未確認） |
| 初回加算 (`shokai_kasan`) | 1件 | 💰❔ 算定中（要件未確認） |
| 生活機能向上連携加算(I) (`seikatsu_kinou_renkei_I`) | 1件 | 💰❔ 算定中（要件未確認） |
| 緊急時訪問介護加算 (`kinkyu_houmon`) | 1件 | 💰❔ 算定中（要件未確認） |
| 介護職員処遇改善加算 (`shougu_kaizen_kasan`) | 6件 | 💰❔ 算定中（要件未確認） |

## 📄❔ PDFから未検出だが取得候補の加算

> **重要**: PDFから検出されないことは「未算定」を断定するものではありません。サービスコード未収載・帳票形式違い・PDF抽出ロジック未対応の場合があります。

- 📄❔ **特定事業所加算(I)** (`tokutei_jigyousho_I`) — ★最重要・経営インパクト最大・年数百万〜数千万円規模
...
- generated_at: `2026-05-11T02:19:07`
...
|---|---|---|---|---|---|---|
| 特定事業所加算(I) | 未検出 | 🟡 partially_clear | ℹ️ 帳票パターン | サ責全員が介護福祉士3年以上 or 実務者研修5年以上 / Aルート tenant_status: 介護福祉士比率30%以上 / Aルート 職員データ集計: 介護福祉士比率30%以上 / Bルート tenant_status: 介護福祉士+実務者+基礎研修50%以上 / Bルート 職員データ集計: 介護福祉士+実務者+基礎研修50%以上 / Aルート 利用者集計: 重度者20%以上 / Bルート tenant_status: 看取り期対応実績1人以上 / Bルート 利用者集計: 看取り期対応関連1件以上 / サ責→ヘルパー指示・報告体制 / サ責健康診断の実施 / 重要事項説明書の緊急時対応記載 / 職員データ集計: 上乗せ配置サ責の常勤換算1.0以上 | tenant_status.monthly_meeting_record.status | ℹ️ pattern_based_unverified |
...
| users_total | 32 |
| care_level_3_or_higher_count | 18 |
| care_level_3_or_higher_ratio | 0.562 |
| care_level_4_or_higher_count | 9 |
| care_level_4_or_higher_ratio | 0.281 |
| severe_user_count | 8 |
| severe_user_ratio | 0.250 |
| dementia_related_count | 9 |
| medical_dependency_count | 3 |
| terminal_care_related_count | 1 |
| discharge_support_related_count | 1 |
| emergency_response_related_count | 4 |
| care_level_distribution | youshien_1: 0 / youshien_2: 0 / youkaigo_1: 6 / youkaigo_2: 8 / youkaigo_3: 9 / youkaigo_4: 6 / youkaigo_5: 3 |
| dementia_care_level_distribution | I: 4 / IIa: 4 / IIb: 5 / IIIa: 4 / IIIb: 3 / IV: 2 / M: 0 |
...
| 集計項目 | 値 |
|---|---|
| saseki_qualified_count | 3 |
| saseki_uwanose_fte | 1 |
| helper_total_count | 7 |
| helper_total_fte | 5 |
| helper_kaigo_fukushishi_count | 3 |
| helper_kaigo_fukushishi_ratio | 0.43 |
| helper_fukushishi_jitsumusha_kiso_ratio | 0.57 |
| helper_qualified_any_count | 6 |
