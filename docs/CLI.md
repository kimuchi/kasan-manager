...
Cloud Run へ初めてデプロイする際の API 有効化・Artifact Registry 作成・Secret 登録・IAM 付与を冪等に実行します。

```bash
npm run setup:gcp                                     # インタラクティブで Secret 登録
npm run setup:gcp -- --gemini-key=AIzaSyXXXXXX        # Secret も同時登録
...
3. Secret Manager に `GEMINI_API_KEY` を登録
4. Cloud Build SA に `roles/run.admin` / `roles/iam.serviceAccountUser` / `roles/artifactregistry.writer` を付与
5. Compute Engine デフォルト SA に `roles/secretmanager.secretAccessor` を付与（Cloud Run 実行時に Secret を読めるようにする）
...
```bash
npm run deploy:cloudrun                # Cloud Build 経由（推奨）
npm run deploy:cloudrun:local          # ローカルで docker build → push → deploy
```

`.env` の `GCP_PROJECT_ID` / `GCP_REGION` / `CLOUD_RUN_SERVICE_NAME` 等を自動で参照します。`cloudbuild.yaml` を経由するのでビルド成果物は Artifact Registry に格納されます。
