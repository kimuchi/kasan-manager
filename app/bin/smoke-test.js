...
// ── PR-2: データ要求エンジン ─────────────────────────────
await test('PR-2: 不足データを加算別・提出方法付きで集約（同一fact_pathは1件に）', async () => {
  const judgeResult = {
    judgements: {
      chujudosha_care_taisei: { name: '中重度者ケア体制加算' },
      k2: { name: '別の加算' },
    },
    dsl_results: {
      chujudosha_care_taisei: {
        status: 'blocked_by_missing_evidence',
        missing_evidence: ['user_summary.care_level_3_or_higher_ratio'],
      },
      k2: {
    ...
    classification: {
      chujudosha_care_taisei: {
        user_visible_bucket: 'needs_more_data',
        next_required_data: ['user_summary.care_level_3_or_higher_ratio'],
      },
      k2: {
        user_visible_bucket: 'claimed_evidence_risk',
        next_required_data: ['user_summary.care_level_3_or_higher_ratio'],
      },
    },
  };
  await attachDataRequests(judgeResult);
  const reqs = judgeResult.data_requests;
  assert.equal(reqs.length, 1, '同一 fact_path は1件に集約されるべき');
  const r = reqs[0];
  assert.equal(r.fact_path, 'user_summary.care_level_3_or_higher_ratio');
  assert.equal(r.priority, 'high', 'claimed_evidence_risk を含むので優先度高');
  assert.equal(r.target_kasans.length, 2);
  assert.equal(r.manual_input_allowed, true);
  assert.equal(r.template_kind, 'user_summary');
  assert.ok(r.acceptable_sources.includes('利用者一覧CSV'));
  assert.ok(r.data_label.includes('要介護3以上'));
});

// ── PR-5: 制度マスタの保留化（AI断定抑止）─────────────────
await test('PR-5: 根拠未確認マスタの注記が付与され、AIプロンプトに断定禁止が出る', async () => {
  const m = await loadMaster('tsusho_kaigo');
  const ks = m.master.kasans;
  assert.equal(ks.kobetsu_kinou_I_i.source_status, 'needs_review_until_official_text_attached');
  assert.ok(ks.kobetsu_kinou_I_i.official_interpretation_note.includes('断定しない'));
  assert.equal(ks.kagakuteki_kaigo.source_status, 'needs_review');
  assert.equal(ks.kagakuteki_kaigo.ai_output_policy, 'frequency_must_not_be_stated_without_source');
  assert.equal(ks.nyuyoku_II.eligible_evaluator_roles.source_status, 'needs_review');
